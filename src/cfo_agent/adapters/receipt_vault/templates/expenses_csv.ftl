...
${report.reportID}|${report.reportName?replace("|","/")}|${report.status}|${report.accountEmail!""}|${expense.created}|${expense.merchant?replace("|","/")}|${(expense.modifiedMerchant!"")?replace("|","/")}|${expense.amount?c}|${(expense.modifiedAmount!0)?c}|${expense.currency}|${(expense.category!"")?replace("|","/")}|${(expense.tag!"")?replace("|","/")}|${expense.billable?c}|${expense.reimbursable?c}|${expense.receiptObject.url!""}|${expense.transactionID!""}<#lt>
  </#list>
</#list>
